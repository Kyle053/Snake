...
class Snake:
    def __init__(self, initLen, initDir, speed):
        ...
        self.speed = speed # Squares per second
    ...
    def state0(self, initLen, gridSize):
        y = round((gridSize[1] - 1) / 2) # Puts snake middle of board

        # Initialises coord list
        rows = initLen
        coords = []
    ...
    def render_snake(self, PPS, window):
        offset = PPS/5 # Thickness of black border
    ...
    def move(self):
        ...
        if now - self.moveTime >= 1/self.speed: # Since speed is squares per second
            ...
            # This moves the head in the direction of 'self.direction'
    ...
    def check_fail(self, gridSize): # Returns True if failed, False otherwise
    ...
    def __init__(self, gridSize):
        ...
        for row in range(rowNum):
            for col in range(colNum):
                # Adds every grid coord to a list
                self.totalSet.append((row, col))

    def state0(self, gridSize):
        # The initial position of the apple is constant
...
def fail_screen(font, width, height, window, previousClick):
    ...
    return previousClick, click # The previosClick stuff is to stop being able to click and THEN move mouse over button
...
def create_button(rect, activeColour, passiveColour, text, font, window, previousClick): # Creates and handles button, rect = (x, y, width, height)
    ...
    if rect[0] <= mouse[0] <= rect[0] + rect[2] and rect[1] <= mouse[1] <= rect[1] + rect[3]: # If mouse over button
        pygame.draw.rect(window, activeColour, rect)
        if click == 1 and previousClick == 0: # So you have to click on the button, not click and hold
            previousClick = click
...
def main():
    gridSize = [20, 15] # Grid size of snake board, doesnt have to be square, at least 4x4 or instant fail
    ...
    while playing:
        for event in pygame.event.get():
            ...
            # Clicking two directions in quick succession is tempermental
                ...
                if event.key == pygame.K_RIGHT:
                    ...
                    snake.direction = (0, -1)
        # This scene is the game playing
        if scene == 0:
        ...
        # This scene is game over
            ...
            # Will be used for settings menu
            pass
